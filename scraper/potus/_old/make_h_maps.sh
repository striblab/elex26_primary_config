...
PARTY="R"
DISTRICT_STR="gop"
MAPSHAPER_COLORS="#c3464a,#e9b04e,#b08c82,#b08c82,#b08c82,#b08c82"
MAPSHAPER_CATEGORIES="Donald J. Trump,Nikki Haley,Vivek Ramaswamy,Ron DeSantis,Chris Christie,Write-In"
...
precincts_voting;votes;votes_pct;votes_office" | \
  cat - <(wget -O - -o /dev/null 'https://electionresultsfiles.sos.mn.gov/20240305/PNPPrecinct.txt') > gop.csv &&

csv2json -s ";" gop.csv | ndjson-cat | \
  ndjson-split | \
  ndjson-filter "d.office_id == \"$OFFICE_ID\" && d.party == \"$PARTY\"" > $DISTRICT_STR.tmp.ndjson &&
