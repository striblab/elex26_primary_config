<script>
  ...
  // Single source of truth for each primary race: who maps to which named hue.
  // Each hue expands into a 3-step light/medium/dark ramp (see HUES/buildRamp),
  // used for BOTH the MapLibre fill-color expression and the legend swatches.

  const HUES = {
    blue: { light: '#CCE5FF', medium: '#48A0FF', dark: '#0064CF' },
    red: { light: '#FECACA', medium: '#F87171', dark: '#D63129' },
    green: { light: '#C4E3D7', medium: '#75AF9B', dark: '#2E725B' },
    orange: { light: '#FBBE7C', medium: '#FB8937', dark: '#B2480B' },
    purple: { light: '#E3DBF0', medium: '#9D86C6', dark: '#5B3C90' },
    yellow: { light: '#FFF2A3', medium: '#FADD4B', dark: '#EAB308' },
    gray: { light: '#E3E5E8', medium: '#A7ADB4', dark: '#737A82' }
  };

  // NOTE: hue assignments below are carried over loosely from the old flat
  // hex colors (e.g. Amy Klobuchar's old blue-ish hex -> 'blue'). Reassign
  // any of these to a different HUES key if you want a different candidate
  // pairing.
  const gopCandidates = [
    { winner: 'Mike Lindell', hue: 'orange' },
    { winner: 'Kendall Qualls', hue: 'yellow' },
    { winner: 'Lisa Demuth', hue: 'green' },
    { winner: 'Loner Blue', hue: 'green' },
    { winner: 'Raul Estrada', hue: 'green' },
    { winner: 'Ross Nova', hue: 'green' },
    { winner: 'John Krhin', hue: 'green' },

    { winner: 'Michele Tafoya', hue: 'orange' },
    { winner: 'Adam Schwarze', hue: 'yellow' },
    { winner: 'Cynthia Gail', hue: 'green' }
    { winner: 'Joyce Lacey', hue: 'green' }
    { winner: 'Royce White', hue: 'green' },
    { winner: 'Ahmad Hassan', hue: 'green' },
    { winner: 'Patrick Munro', hue: 'green' },
    { winner: 'Bob Carney', hue: 'green' },  
    { winner: 'Tom Weiler', hue: 'yellow' }
  ];

  const dflCandidates = [
    { winner: 'Amy Klobuchar', hue: 'purple' },
    { winner: 'Thomas Evanstad', hue: 'green' },
    { winner: 'Mohammed Wazwaz', hue: 'yellow' },
    { winner: 'Kobey Layne', hue: 'yellow' },
    { winner: 'Bill Gates', hue: 'yellow' },
    { winner: 'Po Vang', hue: 'yellow' },
    { winner: 'Ole Savior', hue: 'yellow' },

    { winner: 'Peggy Flanagan', hue: 'purple' },
    { winner: 'Angie Craig', hue: 'green' },
    { winner: 'Kurt Michael Anderson', hue: 'yellow' },
    { winner: 'Peter Murgic', hue: 'yellow' },
    { winner: 'George H. Kalberer', hue: 'yellow' },
    { winner: 'Billy Nord', hue: 'yellow' }
  ];

  // Flat name→color lookup combining both primaries, used by the tooltip
  // to color the dot swatch to match the map's candidate color. Uses each
  // candidate's medium shade as the single representative color.
  const candidateColorMap = new Map(
    [...gopCandidates, ...dflCandidates].map(({ winner, hue }) => [winner, HUES[hue].medium])
  );

  // Margin thresholds for the 3-step ramp: below the first breakpoint is a
  // near-tie, then light/medium/dark map to increasingly decisive margins.
  // Tweak these to match how your winner_margin values are distributed
  // (e.g. percentage points).
  const RAMP_BREAKS = [1, 30, 60];

  /********** COLOR RAMP HELPERS **********/

  // Builds a 3-color light -> dark ramp from a named hue.
  // Index 0 = lightest (weak margin), index 2 = darkest (strong margin).
  function buildRamp(hue) {
    const { light, medium, dark } = HUES[hue];
    return [light, medium, dark];
  }

  // Builds a MapLibre 'case' expression from a candidate list, where each
  // candidate's hue becomes a 3-step margin-based ramp.
  function buildShadesExpr(candidates, tieColor = '#cfcfcf', defaultColor = '#f0f0f0', noDataColor = '#ffffff') {
    ...
    for (const { winner, hue } of candidates) {
  ...
  async function populateDynamicLegend(shading, dataSource) {
    ...
    const present = candidates
      ...
      .map((c) => ({ winner: c.winner, ramp: buildRamp(c.hue) }));
